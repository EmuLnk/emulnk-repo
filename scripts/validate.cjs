...
function scanFile(filePath, patterns, category, exceptions = []) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const pattern of patterns) {
      // Check if any exception matches this line
      if (exceptions.some((exc) => exc.test(line))) continue;

      if (pattern.test(line)) {
        error(`[${category}] ${relFile}:${i + 1}: matches ${pattern}`);
      }
    }
  }
}
...
for (const { absPath, relPath } of allScanPaths) {
  const isTS = fs.existsSync(path.join(absPath, "vite.theme.json"));
  // For TS themes: scan src/ files. For vanilla: scan root files.
  const scanDir = isTS ? path.join(absPath, "src") : absPath;
  const files = collectFiles(scanDir, [".js", ".ts", ".html", ".htm", ".tsx", ".jsx", ".svelte"]);

  for (const file of files) {
    scanFile(file, BANNED_NETWORK, "network");
    scanFile(file, BANNED_EVAL, "eval");
    scanFile(file, BANNED_STORAGE, "storage");
    // External resource check only on HTML files
    if (file.endsWith(".html") || file.endsWith(".htm")) {
      scanFile(file, BANNED_EXTERNAL, "external-resource");
    }
    // Framework scan
    scanFile(file, BANNED_FRAMEWORKS, "framework", FRAMEWORK_EXCEPTIONS);
  }
}
...
for (const { absPath, relPath, id } of themePaths) {
  const isTS = fs.existsSync(path.join(absPath, "vite.theme.json"));
  // For TS themes: measure dist/. For vanilla: measure root files.
  const measureDir = isTS ? path.join(absPath, "dist") : absPath;

  if (isTS && !fs.existsSync(measureDir)) continue; // Not built yet
