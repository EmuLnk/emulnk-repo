...
function parseFieldState(values: BPEValues): FieldState {
...
function parseMons(values: BPEValues): BattleState {
...
export function parseBattle(values: BPEValues): BattleState {
  const inBattle = values.in_battle;

  // Primary gate: gMain.inBattle bit 1 (IWRAM 0x030026F9)
  // Set at battle init, cleared at cleanup — every exit path.
  // Also require gBattleTypeFlags != 0 to avoid false triggers during screen transitions.
  if (inBattle !== undefined) {
    if ((inBattle & 0x02) === 0) return INACTIVE;
    const flags = values.battle_flags ?? 0;
    const outcome = values.battle_outcome ?? 0;
    if (flags === 0 || outcome !== 0) return INACTIVE;
