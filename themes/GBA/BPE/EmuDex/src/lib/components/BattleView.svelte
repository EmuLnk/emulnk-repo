<script lang="ts">
	...
	let playerSpeciesName = $derived(battle.player ? (battle.player.speciesName || `#${battle.player.species}`) : '');
	...
	let player2SpeciesName = $derived(battle.player2 ? (battle.player2.speciesName || `#${battle.player2.species}`) : '');
	...
	<div class="battle-container">
		...
		{#if showCatchRate && !battle.isTrainer && battle.enemy}
			<div class="catch-section">
				<CatchCalc
					species={battle.enemy.internalSpeciesId}
