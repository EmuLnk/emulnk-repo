<script lang="ts">
	...
	import { getSpeciesNameByNatDex, formatEvYield } from "../rom-tables.js";
	...
	// De-duplicate encounters: merge same species+method, combine rate, use widest level range
	let deduped = $derived.by(() => {
		const map = new Map<string, RouteEncounter>();
		for (const enc of encounters) {
			const key = `${enc.method}-${enc.species}`;
				...
				<section class="method-section">
					...
					<div class="pokemon-grid">
						{#each list as enc (cardKey(enc))}
							...
							{@const evStr = status !== "unseen" ? formatEvYield(enc.species) : ''}
							<div
								class="pokemon-card"
