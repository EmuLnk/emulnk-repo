...
import type { BPEValues, Evolution } from "./types.js";
...
let natDexReverse: Record<number, number> | null = null; // national dex → internal ID
...
let evYields: Record<number, number[]> | null = null;
let evolutions: Record<number, Evolution[]> | null = null;
let itemNames: Record<number, string> | null = null;

// Evolutions: 411 species × 5 entries × 8 bytes = 16,440 bytes
const EVO_CHUNKS = [51, 51, 51, 51, 51, 51, 51, 51, 3] as const;
...
const EV_YIELD_OFFSET = 10;         // u16 LE at offset +10 (packed: 2 bits per stat)
...
export function initRomTables(values: BPEValues): void {
  ...
  // --- Catch rates + flee rates + abilities from gSpeciesInfo (Phase 2) ---
  {
    ...
    evYields = hasAny && Object.keys(evs).length > 0 ? evs : null;
  ...
  // --- Evolution table (Phase 2) ---
  {
    let idx = 0;
    let hasAny = false;
    const evoMap: Record<number, Evolution[]> = {};
          ...
          for (let e = 0; e < EVOS_PER_SPECIES; e++) {
            ...
            if (off + 6 <= bytes.length) {
              ...
              if (method > 0 && target > 0) {
                entries.push({ method, param, targetSpecies: target });
              }
            }
          }
          if (entries.length > 0) {
    ...
    evolutions = hasAny && Object.keys(evoMap).length > 0 ? evoMap : null;
  }
...
/** Get EV yield by internal species ID. Returns [HP,Atk,Def,Spd,SpA,SpD] (0-3 each). */
export function getEvYield(internalId: number): number[] | null {
  return evYields?.[internalId] ?? null;
}

/** Get EV yield by national dex number. */
export function getEvYieldByNatDex(natDex: number): number[] | null {
  ...
  return evYields?.[internal] ?? null;
}

const EV_STAT_NAMES = ['HP', 'Atk', 'Def', 'Spd', 'SpA', 'SpD'];

/** Format EV yield as compact string (e.g., "Spd+1 Atk+2"). Empty string if no EVs. */
...
/** Get evolutions by internal species ID. Returns empty array if none. */
export function getEvolutions(internalId: number): Evolution[] {
  return evolutions?.[internalId] ?? [];
}
...
/** Format evolution as readable string. */
